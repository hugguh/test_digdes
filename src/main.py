...
async def flexboxLayout(request):
    app = request.app
    log = app['logging']
    log.warning("flexboxLayout page open")
    data_page = {"error": ""}
    response = aiohttp_jinja2.render_template('flexboxLayout.html', request, data_page)
    return response
...
def main():
    ...
    app.router.add_get('/flexboxLayout', flexboxLayout)
